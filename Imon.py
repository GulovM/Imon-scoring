...
def predict_note_authentication(gender, Issue_amount_nominal, Term, age, Family_status, Type_of_client, education, Type_of_business):
    prediction = classifier.predict([[gender, Issue_amount_nominal, Term, age, Family_status, Type_of_client, education, Type_of_business]])
    return prediction
...
def main():
    ...
    family_status = st.radio('Каков ваш семейный статус?', ('Не женат/замужем', 'Разведен(-а)', 'Женат/Замужем', 'Вдова/Вдовец'))
